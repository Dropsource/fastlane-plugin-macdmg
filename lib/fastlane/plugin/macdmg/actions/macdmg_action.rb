...
require_relative '../helper/macdmg_helper'

module Fastlane
  module Actions
    class MacdmgAction < Action
      def self.run(params)
        ...
        if (params[:add_application_folder_link]) {
          device = Actions.sh "hdiutil attach -readwrite -noverify \"#{absolute_output_path.shellescape}\" | egrep '^/dev/' | sed 1q | awk '{print $1}"
          Actions.sh "sleep 2"
          UI.message "Add link to /Applications"

          Actions.sh "pushd /Volumes/#{params[:volume_name]}"
          ln -s /Applications
          popd
        }
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(key: :size,
                                       ...
                                       type: Integer,
                                       optional: true),
          FastlaneCore::ConfigItem.new(key: :add_application_folder_link,
                                       env_name: "FL_APPLICATION_FOLDER_LINK",
                                       description: "Flag for whether or not to add a link to the /Applications Folder",
                                       default_value: true,
                                       type: Boolean,
                                       optional: true),
          FastlaneCore::ConfigItem.new(key: :background_image,
                                       env_name: "FL_BACKGROUND_IMAGE",
                                       description: "The background image to use in the dmg",
                                       optional: true),
        ]
      end
